...
class LazyJsonProcessedJob(ProcessedJobInterface):
  ...
  def start(self) -> datetime:
    if self._start is None:
      self._start = datetime.fromisoformat(json_reader.ensure_get_str(self._data, "start"))
    return self._start
  
  @property
  def end(self) -> datetime:
    if self._end is None:
      self._end = datetime.fromisoformat(json_reader.ensure_get_str(self._data, "end"))
    return self._end
  ...
  def energy(self) -> float:
    if self._energy is None:
      self._energy = float(json_reader.ensure_get_number(self._data, "energy"))
    return self._energy

  @property
  def emissions(self) -> float:
    if self._emissions is None:
      self._emissions = float(json_reader.ensure_get_number(self._data, "emissions"))
  ...
  def generation_mix(self) -> dict[str, float]:
    if self._generation_mix is None:
      self._generation_mix = {
        json_reader.expect_str(source): float(json_reader.expect_number(value))
        for source, value
        in json_reader.ensure_get_dict(self._data, "generation_mix").items()
      }
