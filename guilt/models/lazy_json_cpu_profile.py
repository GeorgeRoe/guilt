...
class LazyJsonCpuProfile(CpuProfileInterface):
  ...
  def name(self) -> str:
    if self._name is None:
      self._name = JsonReader.ensure_get_str(self._data, "name")
    return self._name
  
  @property
  def tdp(self) -> float:
    if self._tdp is None:
      self._tdp = float(JsonReader.ensure_get_number(self._data, "tdp"))
    return self._tdp
  
  @property
  def cores(self) -> int:
    if self._cores is None:
      self._cores = int(JsonReader.ensure_get_number(self._data, "cores"))
    return self._cores