...
impl Migration for MigrateToProfileResolution {
    fn migrate(&self, user: &User) -> anyhow::Result<()> {
        ...
        let new_unprocessed_jobs: Vec<NewUnprocessedJob> =
            old_unprocessed_jobs.into_iter().map(Into::into).collect();
        ...
        let new_processed_jobs: Vec<OldProcessedJob> = old_processed_jobs
            .into_iter()
            .filter(|job| job.cpu_profile_name != "Default")
            .collect();
        ...
        let new_cpu_profiles: Vec<OldCpuProfile> = old_cpu_profiles
            .into_iter()
    ...
    fn detect_applicable(&self, user: &User) -> anyhow::Result<bool> {
        ...
        let version_str = read_to_string(last_written_version_path)?
            .trim()
            .to_string();
...
mod tests {
    use super::{
        CpuProfileResolutionData, MigrateToProfileResolution, Migration, NewUnprocessedJob,
        OldCpuProfile, OldProcessedJob, OldUnprocessedJob,
    };
    use crate::json_io::{read_json_file, write_json_file};
    ...
    fn migrate_removes_processed_jobs_with_default_cpu() {
        ...
        MigrateToProfileResolution
            .migrate(&testing_user.user)
            .unwrap();

        let guilt_dir = testing_user.user.home_dir.join(".guilt");

        let processed_jobs: Vec<OldProcessedJob> =
            read_json_file(guilt_dir.join("processed_jobs.json")).unwrap();

        assert!(
            processed_jobs
                .iter()
                .all(|job| job.cpu_profile_name != "Default")
        );
    }
    ...
    fn migrate_swaps_default_cpu_jobs_to_no_resolution_data() {
        ...
        let unprocessed_jobs: Vec<NewUnprocessedJob> =
            read_json_file(guilt_dir.join("unprocessed_jobs.json")).unwrap();
    ...
    fn migrate_bumps_last_written_version() {
        ...
        assert_eq!(
            fs::read_to_string(guilt_dir.join("last_written_version")).unwrap(),
            "1.3.0"
        );
    }
}
