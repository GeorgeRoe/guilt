...
impl TerminalPlotter {
    fn plot_map(&self, map: &HashMap<String, f64>) {
        ...
        let longest_key_length = map.keys()
            .map(|key| key.len())
            .max()
            .unwrap_or(0);

        let size = terminal_size();
        let terminal_width = size.map_or(80, |(Width(w), _)| w as usize).saturating_sub(2);
...
impl Plotter for TerminalPlotter {
    ...
    fn draw_intensity_forecast(&self, intensity_forecast: Vec<crate::carbon_intensity_api::CarbonIntensityTimeSegment>) -> Result<(), SomeError> {
        ...
        println!("---------------------");
        self.plot_map(&intensity_forecast.iter()
            .map(|segment| (segment.from.format("%H:%M").to_string(), segment.intensity as f64))
            .collect::<HashMap<_, _>>());
        Ok(())
    }
}