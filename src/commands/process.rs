...
use crate::repositories::{UnprocessedJobsRepository, ProcessedJobsRepository, UserDataRepository};
use crate::slurm::accounting::{get_jobs_by_id, EndTime, StartTime};
...
pub async fn run() -> Result<(), SomeError> {
    ...
    let sacct_results = get_jobs_by_id(
        &unprocessed_jobs.iter().map(|job| job.job_id.clone()).collect::<Vec<_>>(),
    )?.into_iter().map(|result| (result.job_id.clone(), result))
        .collect::<HashMap<_, _>>();
    ...
    for unprocessed_job in unprocessed_jobs {
        if let Some(sacct_result) = sacct_results.get(&unprocessed_job.job_id) {
            if let (
                ...
                let tdp_per_core = unprocessed_job.cpu_profile.tdp as f64 / unprocessed_job.cpu_profile.cores as f64;
                let duration = end_time - start_time;
                let hours = duration.num_seconds() as f64 / 3600.0;
                let energy = (tdp_per_core * cores_used as f64 * hours) / 1000.0;

                let intensity = aggregator.get_average_intensity(start_time, end_time).await?;
                let emissions = energy * intensity;

                println!("Processed job {}: {:.2} kWh, {:.2} gCO2", 
                ...
                let processed_job = ProcessedJob {
                    ...
                    energy: energy,
                    emissions: emissions,
                    generation_mix: aggregator.get_average_generation_mix(start_time, end_time).await?,
                };
