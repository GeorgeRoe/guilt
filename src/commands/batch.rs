...
pub async fn run(job: &str) -> Result<(), SomeError> {
    ...
    let cpu_profile = user_data_repo.get_cpu_profile_by_name(&guilt_directives.cpu_profile)?.ok_or(BatchCommandError::CpuProfileNotFound(guilt_directives.cpu_profile))?;

    let begin: Option<DateTime<Utc>> = if earliest_possible_start_time + slurm_directives.time > latest_forecast_time {
        println!("Your job is extends into unforecasted carbon intensity data. No delays will be applied.");
        None
    } else {
        let tdp_kilowatts = (slurm_directives.tasks_per_node * slurm_directives.cpus_per_task * slurm_directives.nodes) as f64 * (cpu_profile.tdp as f64 / cpu_profile.cores as f64) / 1000.0;
        ...
        aggregator.get_segments(earliest_possible_start_time, latest_forecast_time).await?;

        let earliest_possible_intensity = aggregator.get_average_intensity(earliest_possible_start_time, earliest_possible_start_time + slurm_directives.time).await?;
        ...
        let start_times = (1..(latest_possible_start_time - earliest_possible_start_time).num_minutes() / SEARCH_RESOLUTION)
        ...
        for (start, end) in time_windows {
            let intensity = aggregator.get_average_intensity(start, end).await.unwrap_or(f64::MAX);
        ...
        match best_time {
            Some((start, emissions)) => {
                println!("If you had used 'sbatch', the job would have begun at {} and emitted {:.2} grams of CO2.", earliest_possible_start_time.format("%Y-%m-%d %H:%M"), earliest_possible_emissions);
                println!("By delaying the job to {}, it will emit {:.2} grams of CO2.", start.format("%Y-%m-%d %H:%M"), emissions);
                println!("You are saving {:.2} grams of CO2 by delaying the job.", earliest_possible_emissions - emissions);

                Some(start.clone())
            },
            None => {
                println!("Could not find a better time to run your job, no delays will be applied.");
                None
            },
        }
    };
